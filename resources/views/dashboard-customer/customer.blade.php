        ...
        <section>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach($categories as $category)
                    <a href="#" class="glass-card hover:bg-white/5 p-6 rounded-2xl cursor-pointer group transition-all duration-300 hover:-translate-y-1 text-center flex flex-col items-center gap-3">
                        <div class="w-14 h-14 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center group-hover:scale-110 group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300">
                            @if(Str::contains(strtolower($category->slug), 'concert'))
                                <i data-lucide="mic-2" class="w-7 h-7"></i>
                            @elseif(Str::contains(strtolower($category->slug), 'festival'))
                                <i data-lucide="tent" class="w-7 h-7"></i>
                            @elseif(Str::contains(strtolower($category->slug), 'sport'))
                                <i data-lucide="dribbble" class="w-7 h-7"></i>
                            @elseif(Str::contains(strtolower($category->slug), 'exhibition'))
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <section class="lg:col-span-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    @forelse($pilihanEvents as $event)
                        <div class="glass-card rounded-2xl overflow-hidden group cursor-pointer relative">
                            <div class="absolute top-3 right-3 z-20 glass-panel px-3 py-1.5 rounded-full text-xs font-bold text-white flex items-center gap-1">
                            ...
                            <div class="aspect-video relative overflow-hidden bg-[#1e1e28]">
                                @if($event->banner)
                                    <img src="{{ Str::startsWith($event->banner, 'http') ? $event->banner : asset('storage/' . $event->banner) }}" alt="{{ $event->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                                @else
                                ...
                                @endif
                                <div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#0b0b0f] to-transparent z-10"></div>
                            </div>
                            
                            <div class="p-5 relative z-20 -mt-6">
                                <span class="text-xs font-bold tracking-wider text-[#6a5af9] uppercase mb-1 block">{{ $event->category->name ?? 'Event' }}</span>
                                <h3 class="text-xl font-bold mb-2 group-hover:text-[#6a5af9] transition-colors truncate">{{ $event->title }}</h3>
                                <div class="flex items-center gap-4 text-sm text-gray-400 mb-4">
                                    <span class="flex items-center gap-1.5"><i data-lucide="calendar" class="w-4 h-4"></i> {{ \Carbon\Carbon::parse($event->start_date)->format('M d') }}</span>
                                    <span class="flex items-center gap-1.5 truncate" title="{{ $event->location }}"><i data-lucide="map-pin" class="w-4 h-4"></i> {{ Str::limit($event->location, 15) }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    ...
                                    <a href="{{ url('/events/' . $event->id) }}" class="bg-white/10 hover:bg-[#6a5af9] text-white p-2.5 rounded-xl transition-colors duration-300">
            ...
            <section class="lg:col-span-1">
                ...
                <div class="glass-card rounded-3xl p-6 relative overflow-hidden">
                    ...
                    <div class="relative z-10 border-b border-white/10 pb-5 mb-5">
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1">
                                <span class="text-[10px] sm:text-xs font-bold text-[#ff2d55] bg-[#ff2d55]/10 px-2 py-1 rounded-md mb-1.5 inline-block">TOMORROW</span>
                                <h4 class="font-bold text-white leading-tight mb-1 text-sm sm:text-base">Coldplay: Stadium Tour</h4>
                                <p class="text-[10px] sm:text-xs text-gray-400 flex items-center gap-1.5 mb-2"><i data-lucide="clock" class="w-3 h-3"></i> 19:30 WIB</p>
                                <p class="text-[10px] sm:text-xs text-gray-400 flex items-center gap-1.5"><i data-lucide="map-pin" class="w-3 h-3"></i> GBK</p>
                            </div>
                        </div>
                        <div class="mt-4 pt-4 border-t border-dashed border-white/20 flex justify-between items-center">
                            <div class="text-xs sm:text-sm font-bold font-mono tracking-widest text-[#ff2d55]">#EVT-88492</div>
                            <button class="text-[10px] sm:text-xs font-bold text-white bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-full transition-colors">Show QR</button>
                        </div>
                    </div>
                ...
                <div class="mt-4 p-4 rounded-xl border border-dashed border-white/20 text-center text-sm text-gray-400 bg-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                    + You have 2 more upcoming tickets
                </div>
            </section>
        </div>
    </main>
